...
from .models import YouTubeVideo, OpenAIArticle, AnthropicArticle, XPost, Digest
...
class Repository:
    ...
    def bulk_create_x_posts(self, posts: List[dict]) -> int:
        new_posts = []
        for p in posts:
            existing = self.session.query(XPost).filter_by(guid=p["guid"]).first()
            if not existing:
